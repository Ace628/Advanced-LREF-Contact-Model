...
K_R = 0.8;       % The radial stiffness of the elastic foundation [N/mm]

% Unilateral foundation case
K_R_C = 5e-2*K_R;  % The compressive stiffness of the elastic foundation [N/mm]
K_T = 1e-2*K_R;% The tangential stiffness of the elastic foundation [N/mm]


k_gen = 0.1*K_R; % The contact stiffness
...
% ang_PointF0 = (-pi:D_ang_PointF:pi)';

% [q_Cps_PointF,Rot_Cps_PointF,K_LRNF] = Compensation_Load_NF_PointLoad(R,h,K_R,K_R_C,Point_F,ang_PointF0,D_ang_PointF,U_Cons,Roots,Delta,UTh_Cons,UFi_Cons);
...
Cnt_psiCps = zeros(length(ang_Cps0),1);

% Contact behaviors in the contact region
for j0 = 1:20000 % The maximum iteration step

for i1 = 1:length(ang_i)
    Road(i1) = (R+h/2+he-delta0)/cos(ang_rot(i1));   % Road function for calculating geometry errors
...
[q_Cps,Rot_Cps] = Compensation_Load_NF_NPT2(R,h,he,K_R,K_R_C,D_Cnt_ang,F_i,ang_rot,ang_Cps0,D_ang_Cps,U_Cons,Roots,Delta,UTh_Cons,UFi_Cons,UPsi_Cons);

% The total deformation of the penetration region under F_i and q_Cps
for i2 = 1:length(ang_i)

    for i3 = 1:length(F_i)
        Cnt_urF(i3) = F_i(i3)*(R+h/2+he)*D_Cnt_ang*ur_bar_Laminated_REF(ang_i(i2),ang_rot(i3),U_Cons,Roots,Delta);
        Cnt_utF(i3) = F_i(i3)*(R+h/2+he)*D_Cnt_ang*ut_bar_Laminated_REF(ang_i(i2),ang_rot(i3),U_Cons,Roots,Delta,UTh_Cons);
        Cnt_fiF(i3) = F_i(i3)*(R+h/2+he)*D_Cnt_ang*ufi_bar_Laminated_REF(ang_i(i2),ang_rot(i3),U_Cons,Roots,Delta,UFi_Cons);
        Cnt_psiF(i3) = F_i(i3)*(R+h/2+he)*D_Cnt_ang*upsi_bar_Laminated_REF(ang_i(i2),ang_rot(i3),U_Cons,Roots,Delta,UPsi_Cons);
    end

    for i4 = 1:length(q_Cps)
        Cnt_urCps(i4) = q_Cps(i4)*(R+h/2+he)*D_ang_Cps*ur_bar_Laminated_REF(ang_i(i2),Rot_Cps(i4),U_Cons,Roots,Delta);
        Cnt_utCps(i4) = q_Cps(i4)*(R+h/2+he)*D_ang_Cps*ut_bar_Laminated_REF(ang_i(i2),Rot_Cps(i4),U_Cons,Roots,Delta,UTh_Cons);
        Cnt_fiCps(i4) = q_Cps(i4)*(R+h/2+he)*D_ang_Cps*ufi_bar_Laminated_REF(ang_i(i2),Rot_Cps(i4),U_Cons,Roots,Delta,UFi_Cons);
        Cnt_psiCps(i4) = q_Cps(i4)*(R+h/2+he)*D_ang_Cps*upsi_bar_Laminated_REF(ang_i(i2),Rot_Cps(i4),U_Cons,Roots,Delta,UPsi_Cons);
    end

    Cnt_ur(i2) = sum(Cnt_urF)+sum(Cnt_urCps);
    Cnt_ut(i2) = sum(Cnt_utF)+sum(Cnt_utCps);
    Cnt_fi(i2) = sum(Cnt_fiF)+sum(Cnt_fiCps);
    Cnt_psi(i2) = sum(Cnt_psiF)+sum(Cnt_psiCps);

    r_deform(i2) = sqrt((R+h/2+he+Cnt_ur(i2)+he*Cnt_psi(i2))^2+(Cnt_ut(i2)+(h/2+he)*Cnt_fi(i2))^2);
    theta_di(i2) = ang_i(i2) + atan((Cnt_ut(i2)+(h/2+he)*Cnt_fi(i2))/(R+h/2+he+Cnt_ur(i2)+he*Cnt_psi(i2)));
...
if max(abs(D_err)) < 2e-5      %4e-7 2.5e-6 Predefined threshold
    break
else
    Cnt_UR = Cnt_ur;
...
for i6 = 1:length(F_i)
    F_v(i6) = F_i(i6)*cos(theta_di(i6))*(R+h/2+he)*D_Cnt_ang;
end
...
for j1 = 1:length(ang_j)
    ...
    for j2 = 1:length(F_i)
        ...
        Cnt_PSI2(j2) = F_i(j2)*R_ext*D_Cnt_ang*upsi_bar_Laminated_REF(ang_j(j1),ang_rot(j2),U_Cons,Roots,Delta,UPsi_Cons);
    end
    ...
    for j3 = 1:length(q_Cps)
        ...
        Cnt_PSI3(j3) = q_Cps(j3)*R_ext*D_ang_Cps*upsi_bar_Laminated_REF(ang_j(j1),Rot_Cps(j3),U_Cons,Roots,Delta,UPsi_Cons);
    end

    Cnt_UR1(j1) = sum(Cnt_UR2) + sum(Cnt_UR3);
    Cnt_UT1(j1) = sum(Cnt_UT2) + sum(Cnt_UT3);
    Cnt_FI1(j1) = sum(Cnt_FI2) + sum(Cnt_FI3);
    Cnt_PSI1(j1) = sum(Cnt_PSI2) + sum(Cnt_PSI3);
